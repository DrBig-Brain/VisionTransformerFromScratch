...
class CreatePatches(nn.Module):
    def __init__(self, image_width:int,image_heigh:int, image_channels:int, patch_size:int):
        super().__init__()
    ...
    def forward(self,x):
        ...
        x = x[:,:n_w*self.patch_size,:n_h*self.patch_size]
        x = torch.reshape(x,(batch_size, n_w, self.patch_size, n_h, self.patch_size, self.image_channels))
...
if __name__ == "__main__":
    image = torch.randn(1,354,442,3)
    print(f"before = {image.shape}")
    model = CreatePatches(354,442,3,8)
    image_ = model(image)
